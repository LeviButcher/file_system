    ...
    loop
    {
        ...
        match my_int {
            1 =>
            {
                ...
                stdin().read_line(&mut input_two).expect("Could not read user input");
                input = remove_carriage_return(input);
            ...
            2 =>
            {
                ...
                stdin().read_line(&mut input).expect("Could not read user input");
                input = remove_carriage_return(input);
                let disk_mounted = file_system::FileSystem::mount(&input);
                println!("{:?}", disk_mounted);

            }

            3 =>
            {
                println!("Move on buddy problems happen here");
            }

            4 =>
            {
                let directory_contents = Some(file_system::FileSystem::get_directory()).unwrap();
                //println!("{:?}", directory_contents); //WHY let me unwrap the option pleaseeeeeeeeeeeeeeeeeeeeeeeee
            }

            5 =>
            {
                let diagnostic_information = Some(file_system::FileSystem::get_diagnostic()).unwrap();
                //println!("{:?}", diagnostic_information); This is nightmare fuel
            }
            ...
            8 =>
            {
                ...
                let result = file_system::FileSystem::save_as_file(input_two, file_data);
